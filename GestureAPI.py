...
class Gesture(object):
    ...
    def calc_angles(self):
        ...
        for i in range(self.finger_count):
            ...
            self.angle[i] = abs(math.atan2(
                (self.hand_center[1] - y), (x - self.hand_center[0])) * 180 / math.pi)
...
def DefineGestures():
    ...
    Five.set_finger_pos(
        [(475, 80), (475, 80), (475, 80), (475, 80), (475, 80)])
    Five.calc_angles()
    ...
    if(src1.finger_count == src2.finger_count):
        if(src1.finger_count == 0):
            ...
            if(angle_score < 15):
                length_diff = []
                for i in range(src1.finger_count):
                    len1 = np.sqrt((src1.finger_pos[i][
                                   0] - src1.hand_center[0])**2 + (src1.finger_pos[i][1] - src1.hand_center[1])**2)
                    len2 = np.sqrt((src2.finger_pos[i][
                                   0] - src2.hand_center[0])**2 + (src2.finger_pos[i][1] - src2.hand_center[1])**2)
